...
public class BookServiceTest {
	...
	void init() {
		...
		book = book.builder().bookId(1).title("Linux").author("Saurabh").bookcategoryid(1).publicationDate("2023-02-17")
				.price(250).description("Programming with Linux").bookpublisherid(1).quantityAvailable(50).category(category)
				.publisher(publisher).build();
		book1 = book.builder().bookId(2).title("C Programming").author("Vilas").bookcategoryid(1).publicationDate("2023-02-17")
